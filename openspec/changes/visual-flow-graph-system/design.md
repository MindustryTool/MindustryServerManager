...
`FunctionDescriptor`: stable id derived from the reflected member (`fqcn.methodName`), display name, category (package-derived or annotated), parameter/return `TypeRef`s, overloads distinguished by stable hashes, thread requirement (default MAIN_THREAD, overridable), aliases, deprecation, advisory `advanced` flag. Population is **reflection-first**: a `ReflectionRegistryLoader` walks configured class roots (`mindustry.*`, `arc.*`, `plugin.*`) **lazily on first registry query** (never at plugin startup), turning public methods/constructors/EventType members into descriptors with cached reflective `Invoker`s (bound once per overload via `Method.setAccessible` + `Method.invoke`, upgradeable to `LambdaMetafactory`). Explicit `@GraphFunction`-style annotations remain an opt-in channel for curation/metadata enrichment; nothing requires hand-written wrappers. No runtime eager scans, no facade classes.

### D3. Invocation strategy
All calls dispatch through **cached reflective invokers**: the `Method`/parameter lookup happens once when a graph links against the registry; each execution reuses that bound handle (`method.invoke`). Per-execution *lookup* (Class.getMethod string searches) is what's forbidden on hot paths, not reflection itself. With the VM trust model this keeps every Mindustry method reachable with zero wrapper authoring while staying JIT-friendly enough; if a hot graph ever shows invoke overhead, an invoker can transparently graduate to `LambdaMetafactory` behind the same interface.
